...
<?php } elseif($_GET['type'] == "forum") { ?>版塊
<?php } elseif($_GET['type'] == "group") { ?><?php echo $_G['setting']['navs']['3']['navname'];?>
<?php } elseif($_GET['type'] == "blog") { ?>日誌
<?php } elseif($_GET['type'] == "album") { ?>相冊
...
<h1 class="mt">收藏</h1>
<?php } if($list) { ?>
<form method="post" autocomplete="off" name="delform" id="delform" action="home.php?mod=spacecp&amp;ac=favorite&amp;op=delete&amp;type=<?php echo $_GET['type'];?>&amp;checkall=1" onsubmit="showDialog('確定要刪除選中的收藏嗎？', 'confirm', '', '$(\'delform\').submit();'); return false;">
...
<a class="y" id="a_delete_<?php echo $k;?>" href="home.php?mod=spacecp&amp;ac=favorite&amp;op=delete&amp;favid=<?php echo $k;?>" onclick="showWindow(this.id, this.href, 'get', 0);">刪除</a>
...
<p class="mtm pns">
<label for="chkall" onclick="checkall(this.form, 'favorite')"><input type="checkbox" name="chkall" id="chkall" class="pc vm" />全選</label>
<button type="submit" name="delsubmit" value="true" class="pn vm"><em>刪除選中收藏</em></button>
...
<li<?php echo $actives['forum'];?>><a href="home.php?mod=space&amp;do=favorite&amp;type=forum">版塊</a></li>
<?php if(helper_access::check_module('group')) { ?><li<?php echo $actives['group'];?>><a href="home.php?mod=space&amp;do=favorite&amp;type=group"><?php echo $_G['setting']['navs']['3']['navname'];?></a></li><?php } if(helper_access::check_module('blog')) { ?><li<?php echo $actives['blog'];?>><a href="home.php?mod=space&amp;do=favorite&amp;type=blog">日誌</a></li><?php } if(helper_access::check_module('album')) { ?><li<?php echo $actives['album'];?>><a href="home.php?mod=space&amp;do=favorite&amp;type=album">相冊</a></li><?php } if(helper_access::check_module('portal')) { ?><li<?php echo $actives['article'];?>><a href="home.php?mod=space&amp;do=favorite&amp;type=article">文章</a></li><?php } ?>

...
<div class="pinf">
<?php if($multiple) { ?><strong>多選投票</strong><?php if($maxchoices) { ?>: ( 最多可選 <?php echo $maxchoices;?> 項 )<?php } } else { ?><strong>單選投票</strong><?php } if($visiblepoll && $_G['group']['allowvote']) { ?> , 投票後結果可見<?php } ?>, 共有 <?php echo $voterscount;?> 人參與投票
<?php if(!$visiblepoll && ($overt || $_G['adminid'] == 1 || $thread['authorid'] == $_G['uid']) && $post['invisible'] == 0) { ?>
<a href="forum.php?mod=misc&amp;action=viewvote&amp;tid=<?php echo $_G['tid'];?>" onclick="showWindow('viewvote', this.href)">查看投票參與人</a>
<?php } ?>
</div>
...
距結束還有:
<strong>
<?php if($_G['forum_thread']['remaintime']['0']) { ?><?php echo $_G['forum_thread']['remaintime']['0'];?> 天<?php } if($_G['forum_thread']['remaintime']['1']) { ?><?php echo $_G['forum_thread']['remaintime']['1'];?> 小時<?php } ?>
<?php echo $_G['forum_thread']['remaintime']['2'];?> 分鐘
</strong>
</p>
<?php } elseif($expiration && $expirations < TIMESTAMP) { ?>
<p class="ptmr"><strong>投票已經結束</strong></p>
<?php } ?>

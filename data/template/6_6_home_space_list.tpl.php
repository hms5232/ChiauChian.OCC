...
<?php if($ols[$value['uid']]) { ?><img src="<?php echo IMGDIR;?>/ol.gif" alt="online" title="在線" class="vm" /><?php } if($value['videophotostatus']) { ?>&nbsp;<img src="<?php echo IMGDIR;?>/videophoto.gif" title="視頻認證 已認證" class="vm" /><?php } ?>
</h4>
<p class="maxh">
<?php echo $_G['cache']['usergroups'][$value['groupid']]['grouptitle'];?> <?php g_icon($value[groupid]);?><?php if($value['credits']) { ?>&nbsp;積分數: <?php echo $value['credits'];?><?php } ?>
</p>
<div class="xg1">
<a href="javascript:;" id="interaction_<?php echo $value['uid'];?>" onmouseover="showMenu(this.id);" class="showmenu">互動</a>
<?php if(isset($value['follow']) && $key != $_G['uid']) { ?><span class="pipe">|</span><a href="home.php?mod=spacecp&amp;ac=follow&amp;op=<?php if($value['follow']) { ?>del<?php } else { ?>add<?php } ?>&amp;hash=<?php echo FORMHASH;?>&amp;fuid=<?php echo $value['uid'];?>" id="a_followmod_<?php echo $key;?>" onclick="showWindow('followmod', this.href, 'get', 0)"><?php if($value['follow']) { ?>取消收聽<?php } else { ?>收聽TA<?php } ?></a><?php } if(isset($value['isfriend']) && !$value['isfriend']) { ?><span class="pipe">|</span><a href="home.php?mod=spacecp&amp;ac=friend&amp;op=add&amp;uid=<?php echo $value['uid'];?>" id="a_friend_<?php echo $key;?>" onclick="showWindow('friend_<?php echo $key;?>', this.href, 'get', 0)" title="加為好友">加為好友</a><?php } ?>
</div>

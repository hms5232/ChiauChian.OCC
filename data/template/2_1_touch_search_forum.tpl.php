...
0
|| checktplrefresh('./template/default/touch/search/forum.htm', './template/default/touch/search/pubsearch.htm', 1511791405, '1', './data/template/2_1_touch_search_forum.tpl.php', './template/default', 'touch/search/forum')
|| checktplrefresh('./template/default/touch/search/forum.htm', './template/default/touch/search/thread_list.htm', 1511791405, '1', './data/template/2_1_touch_search_forum.tpl.php', './template/default', 'touch/search/forum')
...
<li><a href="forum.php?forumlist=1" class="icon_threadlist">版塊列表</a></li>
...
EOF;
?><?php $policymsgs .= $p.$policymsg;$p = ', ';?><?php } if($policymsgs) { ?><p>每進行一次搜索將扣除 <?php echo $policymsgs;?></p><?php } ?>
</form>

<?php if(!empty($searchid) && submitcheck('searchsubmit', 1)) { ?><div class="threadlist">
<h2 class="thread_tit"><?php if($keyword) { ?>結果: <em>找到 「<span class="emfont"><?php echo $keyword;?></span>」 相關內容 <?php echo $index['num'];?> 個</em> <?php if($modfid) { ?><a href="forum.php?mod=modcp&amp;action=thread&amp;fid=<?php echo $modfid;?>&amp;keywords=<?php echo $modkeyword;?>&amp;submit=true&amp;do=search&amp;page=<?php echo $page;?>" target="_blank">進入管理面板</a><?php } } else { ?>結果: <em>找到相關主題 <?php echo $index['num'];?> 個</em><?php } ?></h2>
<?php if(empty($threadlist)) { ?>
<ul><li><a href="javascript:;">對不起，沒有找到匹配結果。</a></li></ul>
<?php } else { ?>

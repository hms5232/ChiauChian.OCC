...
0
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/common/seditor.htm', 1511269045, '6', './data/template/6_6_forum_post_infloat.tpl.php', './template/rabbit_occ', 'forum/post_infloat')
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/forum/seccheck_post.htm', 1511269045, '6', './data/template/6_6_forum_post_infloat.tpl.php', './template/rabbit_occ', 'forum/post_infloat')
|| checktplrefresh('./template/default/forum/post_infloat.htm', './template/default/common/seccheck.htm', 1511269045, '6', './data/template/6_6_forum_post_infloat.tpl.php', './template/rabbit_occ', 'forum/post_infloat')
;?><?php include template('common/header'); ?><h3 class="flb">
<em id="return_<?php echo $_GET['handlekey'];?>">
<?php if($_GET['action'] == 'newthread') { ?>發表帖子<?php } elseif($_GET['action'] == 'reply') { ?>參與/回復主題<?php } ?>
</em>
<?php if($_GET['action'] == 'newthread' && $modnewthreads) { ?><span class="needverify">需審核</span><?php } if($_GET['action'] == 'reply' && $modnewreplies) { ?><span class="needverify">需審核</span><?php } ?>
<span>
<a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>')" title="關閉">關閉</a>
</span>
</h3>
...
<?php if(!$sortid) { ?><option value="0">分類信息</option><?php } if(is_array($threadsorts['types'])) foreach($threadsorts['types'] as $tsortid => $name) { if(!empty($modelid) && $threadsorts['modelid'][$tsortid] == $modelid || empty($modelid)) { ?>

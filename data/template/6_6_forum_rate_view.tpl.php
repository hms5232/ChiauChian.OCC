...
<div class="f_c">
<h3 class="flb">
<em id="return_<?php echo $_GET['handlekey'];?>">查看全部評分</em>
<span>
<?php if(!empty($_GET['infloat'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>')" title="關閉">關閉</a><?php } ?>
</span>
</h3>
...
<div class="o pns">
總計:<?php if(is_array($logcount)) foreach($logcount as $id => $count) { ?>&nbsp;<?php echo $_G['setting']['extcredits'][$id]['title'];?> <?php if($count>0) { ?>+<?php } ?><?php echo $count;?> <?php echo $_G['setting']['extcredits'][$id]['unit'];?> &nbsp;
<?php } ?>
</div>

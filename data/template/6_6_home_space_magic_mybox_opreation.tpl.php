...
0
|| checktplrefresh('./template/default/home/space_magic_mybox_opreation.htm', './template/default/common/userabout.htm', 1513095689, '6', './data/template/6_6_home_space_magic_mybox_opreation.tpl.php', './template/rabbit_occ', 'home/space_magic_mybox_opreation')
...
<span><?php if(!empty($_GET['infloat'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>');return false;" title="關閉">關閉</a><?php } ?></span>
</h3>
...
<tr>
<th>贈送留言</th>
<td><textarea name="givemessage" rows="3" class="pt">送您一個<?php echo $magic['name'];?>，<?php echo $magic['description'];?>，希望您能喜歡 </textarea></td>
</tr>
</table>
...
<option value="0">選擇道具</option><?php if(is_array($magiclist)) foreach($magiclist as $magics) { ?><option value="<?php echo $magics['magicid'];?>" <?php echo $magicselect[$magics['magicid']];?>><?php echo $magics['name'];?> - <?php echo $magics['description'];?></option>
<?php } ?>
</select>
</p>
<?php } ?>
...
<p class="xi1"><?php if($magic['useperoid'] == 1) { ?>今天<?php } elseif($magic['useperoid'] == 2) { ?>本周<?php } elseif($magic['useperoid'] == 3) { ?>本月<?php } elseif($magic['useperoid'] == 4) { ?>24 小時內<?php } if($useperoid > 0) { ?>您還能使用 <?php echo $useperoid;?> 次本道具<?php } else { ?>您無法再使用本道具了<?php } ?></p>
<?php } ?>
</div>
</dt>
</dl>
...
<dd class="m"><img src="<?php echo $magic['pic'];?>" alt="<?php echo $magic['name'];?>"></dd>
<dt class="z">
<p class="mtm mbm">出售 <input name="magicnum" type="text" size="2" value="1" class="px pxs" /> 張"<?php echo $magic['name'];?>"</p>
<p class="xw0">
回收價:
<?php if($_G['setting']['extcredits'][$magic['credit']]['unit']) { ?>
<?php echo $_G['setting']['extcredits'][$magic['credit']]['title'];?> <?php echo $discountprice;?> <?php echo $_G['setting']['extcredits'][$magic['credit']]['unit'];?>/張
<?php } else { ?>
<?php echo $discountprice;?> <?php echo $_G['setting']['extcredits'][$magic['credit']]['title'];?>/張
<?php } ?>
</p>
</dt>
</dl>
...
<dd class="m"><img src="<?php echo $magic['pic'];?>" alt="<?php echo $magic['name'];?>"></dd>
<dt class="z">
<p class="mtm mbm">丟棄 <input name="magicnum" type="text" size="2" autocomplete="off" value="1" class="px pxs" /> 張"<?php echo $magic['name'];?>"</p>
...
<?php if($operation == 'give') { ?>
<button class="pn pnc" type="submit" name="operatesubmit" id="operatesubmit" value="true" onclick="return confirmMagicOp(e)"><span>贈送</span></button>
<?php } elseif($operation == 'use') { ?>
...
<?php } elseif($operation == 'drop') { ?>
<button class="pn pnc" type="submit" name="operatesubmit" id="operatesubmit" value="true" onclick="return confirmMagicOp(e)"><span>丟棄</span></button>
<?php } ?>
</div>
...
e = e ? e : window.event;
showDialog('確認該操作', 'confirm', '', 'ajaxpost(\'magicform\', \'return_magics\', \'return_magics\', \'onerror\');');
doane(e);
return false;
}

...
<div class="card_mn">
<div class="avt">
<a href="home.php?mod=space&amp;uid=<?php echo $space['uid'];?>" target="_blank" title="進入<?php echo $space['username'];?>的空間"><?php echo avatar($space[uid],small);?></a>
</div>
<div class="c">
<p class="pbn cl">
<span class="y xg1" style="color:<?php echo $space['group']['color'];?>"<?php if($upgradecredit !== false) { ?> title="積分 <?php echo $space['credits'];?>, 距離下一級還需 <?php echo $upgradecredit;?> 積分"<?php } ?>><?php echo $space['group']['grouptitle'];?></span>
...
<div<?php if($allowupdatedoing) { $scdoingid='scdoing'.random(4);?> id="return_<?php echo $scdoingid;?>" onclick="cardUpdatedoing('<?php echo $scdoingid;?>', 0)"<?php } ?>><?php echo $space['spacenote'];?><?php if(helper_access::check_module('doing') && $allowupdatedoing) { ?> <a href="javascript:;" class="xi2">[更新記錄]</a><?php } ?></div>
...
<a href="home.php?mod=space&amp;diy=yes" class="xi2">裝扮空間</a>
<?php } if(helper_access::check_module('wall')) { ?>
<a href="home.php?mod=space&amp;do=wall" class="xi2">查看留言</a>
<?php } ?>
<a href="home.php?mod=spacecp&amp;ac=avatar" class="xi2">編輯頭像</a>
<a href="home.php?mod=spacecp&amp;ac=profile" class="xi2">更新資料</a>
<?php } else { if(helper_access::check_module('follow')) { ?>
<a href="home.php?mod=spacecp&amp;ac=follow&amp;op=<?php if(!empty($follow)) { ?>del<?php } else { ?>add<?php } ?>&amp;hash=<?php echo FORMHASH;?>&amp;fuid=<?php echo $space['uid'];?>" id="card_followmod_<?php echo $space['uid'];?>" onclick="showWindow(this.id, this.href, 'get', 0)" class="xi2"><?php if(!empty($follow)) { ?>取消收聽<?php } else { ?>收聽TA<?php } ?></a>
...
<?php } ?>
<a href="home.php?mod=spacecp&amp;ac=pm&amp;op=showmsg&amp;handlekey=showmsg_<?php echo $space['uid'];?>&amp;touid=<?php echo $space['uid'];?>&amp;pmid=0&amp;daterange=2" id="a_sendpm_<?php echo $space['uid'];?>" onclick="showWindow('showMsgBox', this.href, 'get', 0)" class="xi2">發送消息</a>
<a href="home.php?mod=spacecp&amp;ac=poke&amp;op=send&amp;uid=<?php echo $space['uid'];?>&amp;handlekey=propokehk_<?php echo $space['uid'];?>" id="a_poke_<?php echo $space['uid'];?>" onclick="showWindow(this.id, this.href, 'get', 0);" class="xi2">打個招呼</a>
<?php if(helper_access::check_module('wall')) { ?>
<a href="home.php?mod=space&amp;uid=<?php echo $space['uid'];?>&amp;do=wall" class="xi2">給我留言</a>
<?php } ?>
...
<a href="<?php if($_G['adminid'] == 1) { ?>admin.php?action=members&operation=search&username=<?php echo $encodeusername;?>&submit=yes&frames=yes<?php } else { ?>forum.php?mod=modcp&action=member&op=edit&uid=<?php echo $space['uid'];?><?php } ?>" target="_blank" class="xi1">編輯用戶</a>
<?php } if(checkperm('allowbanuser')) { ?>
<a href="<?php if($_G['adminid'] == 1) { ?>admin.php?action=members&operation=ban&username=<?php echo $encodeusername;?>&frames=yes<?php } else { ?>forum.php?mod=modcp&action=member&op=ban&uid=<?php echo $space['uid'];?><?php } ?>" target="_blank" class="xi1">禁止用戶</a>
<?php } ?>

...
<h3 class="flb">
<em id="return_mods">選擇了 <?php echo $modpostsnum;?> 篇帖子</em>
<span>
<a href="javascript:;" class="flbc" onclick="hideWindow('mods')" title="關閉">關閉</a>
</span>
</h3>
...
<td class="hasd">
<label onclick="switchitemcp('itemcp_stick')" class="labeltxt">置頂</label>
<div class="dopt">
...
<td class="hasd">
<label onclick="switchitemcp('itemcp_digest')" class="labeltxt">精華</label>
<div class="dopt">
<select name="digestlevel">
<option value="0">解除</option>
<option value="1" <?php echo $digestcheck['1'];?>>精華 1</option>
<?php if($_G['group']['allowdigestthread'] >= 2) { ?>
<option value="2" <?php echo $digestcheck['2'];?>>精華 2</option>
<?php if($_G['group']['allowdigestthread'] == 3) { ?>
<option value="3" <?php echo $digestcheck['3'];?>>精華 3</option>
<?php } } ?>
</select>
</div>
</td>
...
<a href="javascript:;" id="highlight_op_2" onclick="switchhl(this, 2)" class="dopt_i<?php if($stylecheck['2']) { ?> cnt<?php } ?>" style="text-indent:0;text-decoration:none;font-style:italic;" title="文字斜體">I</a>
<a href="javascript:;" id="highlight_op_3" onclick="switchhl(this, 3)" class="dopt_l<?php if($stylecheck['3']) { ?> cnt<?php } ?>" style="text-indent:0;text-decoration:underline;" title="文字加下劃線">U</a>
</div>
</td>
</tr>
<tr class="dopt">
...
<td>
<label onclick="switchitemcp('itemcp_recommend')" class="labeltxt">推薦</label>
<div class="dopt">
<label class="lb"><input type="radio" name="isrecommend" class="pr" value="1" checked="checked" />推薦</label>
...
<td>&nbsp;</td>
<td>
<label for="reducetitle" class="labeltxt">標題</label>
...
<td>&nbsp;</td>
<td>
<label class="labeltxt">圖片</label>
<select name="selectattach" onchange="updateimginfo(this.value)" class="ps" style="width: 132px">
<option value="">不顯示</option><?php if(is_array($imgattach)) foreach($imgattach as $imginfo) { ?><option value="<?php echo $imginfo['aid'];?>"<?php if($selectattach == $imginfo['aid']) { ?> selected="selected"<?php } ?>><?php echo $imginfo['filename'];?></option>
<?php } ?>
</select>
</td>
</tr>
...
<p class="mbn tahfx">
目標版塊: <select name="moveto" id="moveto" class="ps vm" onchange="ajaxget('forum.php?mod=ajax&action=getthreadtypes&fid=' + this.value, 'threadtypes');if(this.value) {$('moveext').style.display='';} else {$('moveext').style.display='none';}">
...
<li class="wide"><label><input type="radio" name="type" class="pr" value="normal" checked="checked" />移動主題</label></li>
<li class="wide"><label><input type="radio" name="type" class="pr" value="redirect" />保留轉向</label></li>
</ul>
<?php } else { if($typeselect) { ?>
<input type="hidden" name="operations[]" value="type" />
<p>分類: <?php echo $typeselect;?></p>
<?php } else { ?>
當前版塊無分類設置，請聯繫管理員到後台設置主題分類<?php $hiddensubmit = true;?><?php } } ?>
</div>
...
<p>您確認要 <strong>刪除</strong> 選擇的主題麼?</p>
<?php } else { ?>
<p>您沒有刪除此主題權限</p>
<?php } ?>
</li>
...
<td class="hasd"><label onclick="switchitemcp('itemcp_bump');" class="labeltxt" style="width:50px;margin:5px 0 10px 0px;">提升主題</label></td>
</tr>
<tr class="dopt">
<td>&nbsp;</td>
<td>
...
<td class="hasd"><label onclick="switchitemcp('itemcp_down');" class="labeltxt" style="width:50px;margin:5px 0 10px 0px;">下沉主題</label></td>
</tr>
<!--
<tr class="dopt">
<td>&nbsp;</td>
...
<ul class="llst">
<li class="wide"><label><input type="radio" name="operations[]" class="pr" value="open" <?php echo $closecheck['0'];?>  onclick="$('expirationclose').value='';" />打開主題</label></li>
<li class="wide"><label><input type="radio" name="operations[]" class="pr" value="close" <?php echo $closecheck['1'];?> />關閉主題</label></li>
</ul>
</td>
</tr>
</table>
...
<p class="mbn tahfx">
目標版塊: <select id="moveto" name="moveto" class="ps vm">
<?php echo $forumselect;?>
</select>
</p>
<?php } ?>
...
<button type="submit" name="modsubmit" id="modsubmit" class="pn pnc" value="確定"><span>確定</span></button>
</p>
<?php } ?>
</form>
</div>

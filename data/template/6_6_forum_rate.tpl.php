...
<h3 class="flb">
<em id="return_rate">評分</em>
<span>
<?php if(!empty($_GET['infloat'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('rate')" title="關閉">關閉</a><?php } ?>
</span>
</h3>
...
<button name="ratesubmit" type="submit" value="true" class="pn pnc"><span>確定</span></button>
</p>
</form>
</div>
...
<div class="o pns">
<label class="z" onclick="checkall(this.form, 'logid')"><input class="pc" type="checkbox" name="chkall" />全選</label>
<label for="sendreasonpm"><input type="checkbox" name="sendreasonpm" id="sendreasonpm" class="pc"<?php if($_G['group']['reasonpm'] == 2 || $_G['group']['reasonpm'] == 3) { ?> checked="checked" disabled="disabled"<?php } ?> />通知作者</label>
操作說明: <input name="reason" class="px vm" />
...
<a href="javascript:;" class="xi1" onclick="hideWindow('<?php echo $_GET['handlekey'];?>');showWindow('rate', 'forum.php?mod=misc&action=rate&tid=<?php echo $_GET['tid'];?>&pid=<?php echo $_GET['pid'];?>', 'get', -1);return false;">點擊這裡</a> 給本帖加分
</p>
</div>
</div>
<p class="o pns">
<button onclick="hideWindow('rate');" id="closebtn" class="pn pnc" type="button" fwin="rate"><strong>關閉</strong></button>
</p>

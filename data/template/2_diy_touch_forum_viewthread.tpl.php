...
0
|| checktplrefresh('./template/default/touch/forum/viewthread.htm', './template/default/touch/forum/forumdisplay_fastpost.htm', 1511243955, 'diy', './data/template/2_diy_touch_forum_viewthread.tpl.php', './template/default', 'touch/forum/viewthread')
|| checktplrefresh('./template/default/touch/forum/viewthread.htm', './template/default/touch/common/seccheck.htm', 1511243955, 'diy', './data/template/2_diy_touch_forum_viewthread.tpl.php', './template/default', 'touch/forum/viewthread')
;?>
...
<div class="nav">
<span class="y">
<button class="btn_pn btn_pn_blue" id="replyid"><span>回復</span></button>
</span>
...
<?php echo $_G['forum_thread']['subject'];?>
            <?php if($_G['forum_thread']['displayorder'] == -2) { ?> <span>(審核中)</span>
...
<a href="forum.php?mod=viewthread&amp;tid=<?php echo $_G['tid'];?>&amp;page=<?php echo $page;?>&amp;authorid=<?php echo $_G['forum_thread']['authorid'];?>" rel="nofollow" class="blue" style="font-size:12px;font-weight:normal;margin-left:10px;">只看樓主</a>
...
<a href="javascript:;">遊客 <em><?php echo $post['useip'];?><?php if($post['port']) { ?>:<?php echo $post['port'];?><?php } ?></em></a>
...
<input type="button" value="編輯" class="redirect button" href="forum.php?mod=post&amp;action=edit&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;pid=<?php echo $post['pid'];?><?php if($_G['forum_thread']['sortid']) { if($post['first']) { ?>&amp;sortid=<?php echo $_G['forum_thread']['sortid'];?><?php } } if(!empty($_GET['modthreadkey'])) { ?>&amp;modthreadkey=<?php echo $_GET['modthreadkey'];?><?php } ?>&amp;page=<?php echo $page;?>">
<?php } ?>
<input type="button" value="刪除" class="dialog button" href="forum.php?mod=topicadmin&amp;action=moderate&amp;fid=<?php echo $_G['fid'];?>&amp;moderate[]=<?php echo $_G['tid'];?>&amp;operation=delete&amp;optgroup=3&amp;from=<?php echo $_G['tid'];?>">
<input type="button" value="關閉" class="dialog button" href="forum.php?mod=topicadmin&amp;action=moderate&amp;fid=<?php echo $_G['fid'];?>&amp;moderate[]=<?php echo $_G['tid'];?>&amp;from=<?php echo $_G['tid'];?>&amp;optgroup=4">
...
<div class="message">
                    ...
                    <?php if($_G['adminid'] != 1 && $_G['setting']['bannedmessages'] & 1 && (($post['authorid'] && !$post['username']) || ($post['groupid'] == 4 || $post['groupid'] == 5) || $post['status'] == -1 || $post['memberstatus'])) { ?>
                        <div class="grey quote">提示: <em>作者被禁止或刪除 內容自動屏蔽</em></div>
                    <?php } elseif($_G['adminid'] != 1 && $post['status'] & 1) { ?>
                        <div class="grey quote">提示: <em>該帖被管理員或版主屏蔽</em></div>
                    <?php } elseif($needhiddenreply) { ?>
                        <div class="grey quote">此帖僅作者可見</div>
                    <?php } elseif($post['first'] && $_G['forum_threadpay']) { include template('forum/viewthread_pay'); } else { ?>
                    	...
                    	<?php if($_G['setting']['bannedmessages'] & 1 && (($post['authorid'] && !$post['username']) || ($post['groupid'] == 4 || $post['groupid'] == 5))) { ?>
                            <div class="grey quote">提示: <em>作者被禁止或刪除 內容自動屏蔽，只有管理員或有管理權限的成員可見</em></div>
                        <?php } elseif($post['status'] & 1) { ?>
                            <div class="grey quote">提示: <em>該帖被管理員或版主屏蔽，只有管理員或有管理權限的成員可見</em></div>
                        <?php } ?>
                        <?php if($_G['forum_thread']['price'] > 0 && $_G['forum_thread']['special'] == 0) { ?>
                            付費主題, 價格: <strong><?php echo $_G['forum_thread']['price'];?> <?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit'];?><?php echo $_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title'];?> </strong> <a href="forum.php?mod=misc&amp;action=viewpayments&amp;tid=<?php echo $_G['tid'];?>" >記錄</a>
                        <?php } ?>
...
<?php if($_G['setting']['mobile']['mobilesimpletype'] == 0) { if($post['attachment']) { ?>
               ...
               附件: <em><?php if($_G['uid']) { ?>您所在的用戶組無法下載或查看附件<?php } else { ?>您需要<a href="member.php?mod=logging&amp;action=login">登錄</a>才可以下載或查看附件。沒有帳號？<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" title="註冊帳號"><?php echo $_G['setting']['reglinkname'];?></a><?php } ?></em>
               </div>
...
<input type="button" class="redirect button" href="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;repquote=<?php echo $post['pid'];?>&amp;extra=<?php echo $_GET['extra'];?>&amp;page=<?php echo $page;?>" value="回復">
</div>
<?php } ?>
       </div>
   </div>
...
<?php } ?>
<li><input type="text" value="我也說一句" class="input grey" color="gray" name="message" id="fastpostmessage"></li>
...
<?php } ?><input type="button" value="回復" class="button2" name="replysubmit" id="fastpostsubmit"><a href="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;reppost=<?php echo $_G['forum_firstpid'];?>&amp;page=<?php echo $page;?>" class="y" style="height:30px;width:30px;margin-top:7px;background:url(<?php echo STATICURL;?>image/mobile/images/icon_photo.png) no-repeat"><span class="none">回復</span></a><?php if(!empty($_G['setting']['pluginhooks']['viewthread_fastpost_button_mobile'])) echo $_G['setting']['pluginhooks']['viewthread_fastpost_button_mobile'];?></li>
</ul>
</div>
    </form>
</div>
...
<?php if(!$_G['uid']) { ?>
popup.open('您還未登錄，立即登錄?', 'confirm', 'member.php?mod=logging&action=login');
<?php } else { ?>
popup.open('您暫時沒有權限發表', 'alert');
<?php } ?>
this.blur();
});
<?php } else { ?>
...
<a href="javascript:;" title="返回頂部" class="scrolltop bottom"></a><?php include template('common/footer'); ?>
...
<span><?php if($_G['uid'] == $space['uid']) { ?>我的資料<?php } else { ?><?php echo $space['username'];?>的資料<?php } ?></span>
    </div>
</header>
<!-- header end -->
<!-- userinfo start -->
...
<div class="user_box">
<ul>
<li><span><?php echo $space['credits'];?></span>積分</li><?php if(is_array($_G['setting']['extcredits'])) foreach($_G['setting']['extcredits'] as $key => $value) { if($value['title']) { ?>
<li><span><?php echo $space["extcredits$key"];?> <?php echo $value['unit'];?></span><?php echo $value['title'];?></li>
<?php } } ?>
</ul>
</div>
<?php if($space['uid'] == $_G['uid']) { ?>
<div class="btn_exit"><a href="member.php?mod=logging&amp;action=logout&amp;formhash=<?php echo FORMHASH;?>">退出登錄</a></div>
<?php } ?>
</div>
<!-- userinfo end -->
...
<li class="on" id="usermsg"><a href="<?php if($_G['uid']) { ?>home.php?mod=space&uid=<?php echo $_G['uid'];?>&do=profile&mycenter=1<?php } else { ?>member.php?mod=logging&action=login<?php } ?>" class="icon_userinfo">用戶信息</a><?php if($_G['member']['newpm']) { ?><span class="icon_msg"></span><?php } ?></li>
...
<li><a href="home.php?mod=space&amp;uid=<?php echo $_G['uid'];?>&amp;do=thread&amp;view=me">我的主題</a></li>
...
<li><a href="home.php?mod=space&amp;uid=<?php echo $_G['uid'];?>">我的資料</a></li>
</ul>
</div>
</div>
<!-- userinfo end -->
